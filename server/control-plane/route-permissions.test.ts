...
describe("routed procedure authorization contract", () => {
  it("contains every workspace-scoped operation with an explicit role requirement", () => {
    ...
    expect(routedProcedurePermissions["assurance.decidePolicy"]).toBe(
      "admin-or-distinct-reviewer"
    );
    expect(routedProcedurePermissions["notification.deliveryLedger"]).toBe(
      "self"
    );
    expect(routedProcedurePermissions["assurance.comparePolicies"]).toBe(
      "read-member"
    );
    expect(routedProcedurePermissions["assurance.createIncident"]).toBe(
      "responder"
    );
    expect(routedProcedurePermissions["assurance.incidentReview"]).toBe(
      "read-member"
    );
    expect(routedProcedurePermissions["assurance.saveIncidentReview"]).toBe(
      "responder"
    );
    expect(routedProcedurePermissions["audit.uploadEvidence"]).toBe("owner");
    expect(routedProcedurePermissions["rehearsal.listRuns"]).toBe(
      "read-member"
    );
    expect(routedProcedurePermissions["evidence.duplicateCandidates"]).toBe(
      "read-member"
    );
    ...
    expect(routedProcedurePermissions["knowledge.upsertNode"]).toBe(
      "responder"
    );
    expect(
      routedProcedurePermissions["research.promoteObservationToFinding"]
    ).toBe("responder");
    expect(
      routedProcedurePermissions["research.enqueueIntelligenceFetch"]
    ).toBe("responder");
    expect(routedProcedurePermissions["research.playbookRuns"]).toBe(
      "read-member"
    );
    expect(routedProcedurePermissions["research.transitionPlaybookRun"]).toBe(
      "responder"
    );
    expect(routedProcedurePermissions["organization.createSubmission"]).toBe(
      "responder"
    );
    ...
    expect(routedProcedurePermissions["ai.evaluateRun"]).toBe(
      "distinct-reviewer"
    );
