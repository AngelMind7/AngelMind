...
import { assertPassivePlaybookTaskType, compareAssetSnapshots, normalizeIntelligenceFeed, validateFailureObservation, type AssetSnapshot, type FailureObservation, type IntelligenceFeedItem } from "./control-plane/intelligence-engine";
...
async function audit(db: NonNullable<Awaited<ReturnType<typeof getDb>>>, workspaceId: number, userId: number, subject: string, details: Record<string, unknown>) {
  const { auditEvents } = await import("../database/schema");
...
export async function runPlaybook(userId: number, input: { workspaceId: number; sessionId: number; playbookId: number }) {
  ...
  for (let index = 0; index < templates.length; index += 1) {
    ...
    const type = assertPassivePlaybookTaskType(String(template.type ?? template.taskType ?? "research"));
    if (title.length < 2 || title.length > 240) throw new Error(`Template task ${index + 1} memiliki title/type tidak valid.`);
