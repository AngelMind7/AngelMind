...
for (const tool of requiredTools) if (!catalog.includes(`"toolKey":"${tool}"`) && !catalog.includes(`"toolKey": "${tool}"`) && !catalog.includes(`"toolKey":"${toolAliases[tool] ?? tool}"`) && !catalog.includes(`"toolKey": "${toolAliases[tool] ?? tool}"`)) failures.push(`missing master tool ${tool}`);
requireAtLeast((catalog.match(/"toolKey"\s*:/g) ?? []).length, 50, "UTF catalog modules");
...
console.log(`Master contract OK: ${routes.length} routes, ${domainDocs.length} domains, ${(catalog.match(/"toolKey"\s*:/g) ?? []).length} UTF modules, ${schemas.length} evidence schemas, >=47 correlation rules, ${migrations.length} migrations.`);
