...
async function startServer() {
  validateRuntimeConfig(); const app = express(); const server = createServer(app); registerSecurityMiddleware(app); registerApiRateLimit(app); registerHealthRoutes(app); registerMetricsRoute(app); app.use(express.json({ limit: "8mb" })); app.use(express.urlencoded({ limit: "1mb", extended: true })); registerFirebaseAuthRoutes(app); registerRestV1Routes(app); registerRestV1CoreResourceRoutes(app); registerRestV1TagsNotesRoutes(app); registerRestV1EvidenceFindingRoutes(app); registerToolRestV1Routes(app); registerSimulationRoutes(app); registerRedTeamRestV1Routes(app); registerPurpleTeamRestV1Routes(app); registerBugBountyRestV1Routes(app); registerRestV1ReportingRoutes(app); registerThreatIntelligenceRestV1Routes(app); registerAiAutomationRestV1Routes(app); registerRealtimeRoutes(app); registerRealtimeWebSocket(server); app.post("/api/scheduled/workspace-maintenance", workspaceMaintenanceHandler); app.use("/api/trpc", createExpressMiddleware({ router: appRouter, createContext }));
