...
for (const marker of ["LLM", "WorkerRole", "PromptVersion", "budgetCents", "timeoutSeconds", "simulation", "governed", "approval", "audit"]) if (!service.toLowerCase().includes(marker.toLowerCase())) failures.push(`AI automation marker missing: ${marker}`);
...
if (!service.includes('mode: "simulation"')) failures.push("simulation must remain the default execution mode");
