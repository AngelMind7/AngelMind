...
const failures = [];
for (const file of ["src/c2/server/redteam-operations.ts", "src/c2/server/redteam-operations.test.ts", "src/c2/server/rest-v1-redteam.ts", "docs/domain/07-red-team.md"]) if (!existsSync(resolve(root, file))) failures.push(`missing ${file}`);
const service = read("src/c2/server/redteam-operations.ts");
...
const rest = read("src/c2/server/rest-v1-redteam.ts");
for (const route of [
