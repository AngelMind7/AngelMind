...
| 96 | CI/CD | **Partial** | CI check/test/build/security workflows, guarded DAST, post-deploy staging verification, and a manual promotion workflow requiring staging health/readiness, expected commit, immutable signed image digest, and explicit confirmation are implemented; provider-specific production deployment remains environment-gated. |
