...
| 16 | Asset intelligence | **Partial** | Passive assets/import, deterministic hostname dedupe, discovery delta, first/last-seen timestamps, SHA-256 discovery summaries, scope filtering, four-method target verification, relationship records, cursor-paginated asset listing and signal history, and aggregated domain/technology/service/history inventory are implemented; automatic discovery scheduling and shadow alerts remain open. |
| 17 | Attack-surface model | **Deferred** | Active discovery tetap tidak diaktifkan; passive discovery delta boundary tersedia, tetapi SurfaceNode/exposure model relasional offline, subdomain history, certificate/cloud/code-leak monitoring belum lengkap. |
| 18 | Task engine | **Partial** | Task persistence, revision-safe lifecycle, dependency gate, durable enqueue, lease/heartbeat/retry/dead-letter worker execution, passive adapter boundary, and observation persistence are implemented; broad adapter coverage and production worker verification remain open. |
| 19 | Task dependency graph | **Partial** | Dependency validation, deterministic cycle-safe graph/readiness layout, dependency-ready bounded parallel enqueue, task status transitions, durable worker orchestration, and versioned task lifecycle outbox events are implemented; production multi-process worker verification remains open. |
| 20 | Hypothesis engine | **Partial** | AI evidence analyst mengeluarkan hypotheses; persisted lifecycle transitions, concurrency guard, outbox events, and API are implemented, while evidence-backed validation UI and richer hypothesis review remain open. |
| 21 | Observation engine | **Partial** | Entity Observation, session linkage, cursor pagination, promotion to finding, persisted new/reviewed/linked/archived transitions, authorization, audit, and outbox events are implemented; richer observation review UI remains open. |
...
| 23 | Evidence provenance | **Partial** | Storage reference/hash/audit, external source capture, worker acquisition provenance, research-node edges, source-artifact → target-artifact transformation lineage, workspace-scoped immutable audit-chain verification, and workspace-scoped provenance replay backend are implemented; full acquisition replay UI remains incomplete. |
...
| 30 | Retest | **Partial** | `findingRetests`, retest evidence/result, relation linking, status synchronization, Findings UI request/result workflow, inline evidence upload, and explicit `verified_fixed`/`still_present` terminal outcomes are implemented; browser E2E against staging remains environment-dependent. |
| 31 | Knowledge graph | **Partial** | Workspace-scoped relationship/evidence/finding records, search documents, intelligence feed persistence, and as-of temporal node/edge traversal/listing are implemented; generic graph-node/edge authoring and full graph UI remain incomplete. |
...
| 42 | Reputation | **Partial** | Workspace-scoped reputation events, deterministic points/levels, persisted profiles/achievements, leaderboard/profile API, validation, authorization, dan automatic hooks untuk finding validation/review/evidence/retest tersedia; full domain-wide hooks dan public researcher profile masih terbuka. |
...
| 46 | Integrations | **Partial** | Provider-neutral GitHub/GitLab/Slack/Discord/custom connection lifecycle dengan persisted status, scopes, HTTPS endpoint validation, secret-reference boundary, owner authorization, API, dan Settings UI tersedia; OAuth handshakes, live delivery adapters, dan provider verification tetap environment-dependent. |
...
| 50 | AI failure isolation | **Partial** | Provider fallback, provider-specific fallback model, bounded transient retry statuses, bounded error handling, failure classification, context reduction guidance, human-review escalation for partial/contradictory results, fail-closed recovery caps, per-provider closed/open/half-open circuit breaking, durable DB-backed state with probe leases, monotonic fencing epochs/writer-region metadata, idempotent outbox operational alerts, notification preference routing UI, and an admin circuit diagnostics dashboard are implemented; external consensus beyond the shared database remains incomplete. |
...
| 52 | AI provenance | **Partial** | AI output disimpan sebagai finding/report draft; workspace-authorized `ai.provenance` now exposes task–run–model–input–output lineage, trace, references, and non-sensitive SHA-256 fingerprints; persisted provider-level span lineage remains incomplete. |
...
| 55 | AI result pipeline | **Partial** | Structured JSON output, validation, normalize/deduplicate/correlate/synthesis primitives, persisted completed-run synthesis endpoint, provenance hashes, and human-review gate are implemented; durable synthesized-result storage and provider-wide adoption remain open. |
...
| 57 | Prompt management | **Implemented (repository)** | Versioned JSON prompt registry, role/status validation, active-prompt guard, bounded variable rendering, and authenticated REST registration/listing are available; external prompt governance and provider quality review remain operational concerns. |
| 58 | AI cost governance | **Partial** | Workspace/session controls, deterministic provider/model token-cost calculation, finite-number fail-safe handling, workspace governance summary by provider/model/user/task, and persisted usage accounting are implemented; hard per-user/task budget enforcement and runaway detection remain open. |
| 59 | Job system | **Partial** | Durable job/outbox foundations, centralized enqueue contract, registry-backed scheduled maintenance runner, rehearsal records, orchestration task queueing, and worker contracts exist; remaining domain-wide coverage is limited to intentional ledger writes and provider-specific integrations. |
...
| 61 | Scheduler | **Partial** | Registry-backed runner now executes workspace maintenance, approval expiry, AI-memory retention, and stale outbox recovery jobs with bounded handlers; external trigger/lease observability and production cadence verification remain open. |
...
| 63 | Event architecture | **Partial** | Versioned event schema, task/hypothesis/observation/retest lifecycle publication, outbox lease/retry/consumer receipt, audit/notification events, and realtime publication boundaries are implemented; complete domain-wide publisher/consumer adoption remains open. |
| 64 | Outbox pattern | **Partial** | Versioned outbox schema, event records, leases, consumer receipts, retry/backoff, bounded dispatcher, stale-lease recovery, and research/task/hypothesis/observation/retest lifecycle publication exist; complete production publisher/consumer coverage and operational delivery monitoring remain incomplete. |
...
| 69 | Pagination | **Partial** | Workspace-scoped list queries, global-search cursor pagination, organization/member/program cursor pagination, asset-signal pagination, and organization role-audit cursor pagination with actor/member/role/date filters and CSV export are implemented with deterministic timestamp/ID continuation; remaining domain-wide adoption is incomplete. |
...
| 73 | Search index | **Partial** | Workspace-scoped `searchDocuments` index, rebuild/reindex, permission checks, query scoring, entity-type/freshness filters, REST read-only search, saved views, workspace notes/knowledge/report-draft/finding/evidence/AI-memory mutation indexing, stale-entity delete cleanup, cursor pagination, and unified cross-domain search UI are implemented; semantic/vector retrieval remains an optional enhancement. |
...
| 75 | Privacy | **Partial** | Retention, workspace isolation, legal surfaces, privacy request state machine, privacy center UI, durable export/delete processing, owner guard, private export artifact, owner-only signed download, request audit events, expanded owned-organization/workspace and unsubscribe-preference export coverage, and processing-state/idempotency guards are implemented; live integration drills and provider-backed deletion verification remain open. |
| 76 | Abuse protection | **Partial** | Body limits, safe boundaries, allowlists, route-specific API rate limiting, credential-safe client keys, repeated-violation cooldowns, bounded behavioral risk scoring, credential-variant signals, hashed admin diagnostics, and no target execution are implemented; upload malware provider and account-level identity correlation remain open. |
...
| 79 | Encryption | **Partial** | TLS/security headers, managed at-rest providers, AES-256-GCM audit-state protection, and versioned envelope encryption for MFA secrets with previous-key rotation support are implemented; centralized KMS/HSM-backed key management and live rotation drills remain environment-gated. |
...
| No. | Requirement | Status | Bukti/gap utama |
|---:|---|---|---|
| 90 | Observability | **Implemented (repository)** | Health, readiness, Prometheus process/purge/HTTP counters, request/trace correlation headers, route status and latency/error/slow-request signals, bounded provider probes, SLO budget gauges, logs, audit, versioned Prometheus alert rules, Alertmanager routing/inhibition, and a release-readiness monitoring contract are implemented; external webhook credentials and hosted dashboard provisioning remain environment-dependent. |
| 91 | Traceability | **Partial** | Request, durable job, AI run, execution progress, audit, and database correlation fields now carry bounded trace IDs with generated UUID fallbacks; provider-level distributed tracing and full authenticated E2E trace assertions remain deployment/test-gated. |
...
| 93 | Status page | **Implemented (repository)** | Public posture/status disclosure dan redacted `/statusz` component telemetry dengan loading/error/retry UI tersedia; uptime history dan hosted incident feed tetap deployment-dependent. |
| 94 | Disaster recovery | **Implemented (repository)** | Signed archive, constant-time integrity verification, durable idempotent restore-drill ledger, managed-storage retrieval, manifest parsing, record-count verification, scheduled encrypted backup hook with retention, checksum validation, isolated non-production restore drill, and RTO/RPO evidence are implemented; provider recovery credentials and live restore execution remain environment-dependent and require explicit human confirmation. |
| 95 | Migration | **Implemented (repository)** | Drizzle forward migrations, journal/safety/rollback validation, protected staging/production automatic migration workflow, backup checkpoint hook, concurrency lock, and migration evidence artifacts are implemented; provider backup/PITR execution remains environment-gated. |
| 96 | CI/CD | **Partial** | CI check/test/build/security workflows, guarded DAST, post-deploy staging verification, signed-image promotion, and a manual production deployment workflow using GitHub `production` environment approval, authenticated provider hook, expected commit checks, and post-deploy health/readiness verification are implemented; provider credentials and live deployment remain environment-gated. |
| 97 | Supply-chain security | **Partial** | Lockfile, dependency review/security workflow, pinned CycloneDX SBOM generation/upload, pinned HIGH/CRITICAL Trivy image scan evidence, keyless Cosign signing/verification on main image pushes, guarded ZAP baseline/full DAST workflows, and container hardening are implemented; live staging execution and production registry retention policy remain environment-gated. |
| 98 | Testing | **Partial** | Unit/integration/property/E2E/Python tests, public accessibility axe smoke suite, authenticated core-route axe contract, asset verification tests, and passive delta tests tersedia; authenticated security/performance/DR suites lengkap belum. |
