...
| 109 | Email system | **Partial** | Generic SMTP adapter, typed environment configuration, invitation/reset/verification templates, Indonesian/English locale fallback, durable `emailDeliveries` ledger, idempotent enqueueing, retry-compatible worker status, organization invitation queue wiring, and reusable password-reset/account-verification queue helpers are implemented; identity-provider flow wiring, unsubscribe, and provider-level verification remain open. |
