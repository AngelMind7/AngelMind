...
Python-based passive tooling is deliberately separated into `Dockerfile.research`, based on Python 3.12. The image installs the pinned `checkdmarc==5.17.5` and `dnspython==2.8.0` dependencies from `apps/orchestrator-langgraph/`. This avoids pretending that a Node image can provide the correct Python runtime and keeps Python dependencies out of the web process. Deploy it as a separate Railway worker/service only after adding a scoped worker command and service-level network policy.
