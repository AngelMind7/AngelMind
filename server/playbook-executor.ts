...
async function recordAudit(db: NonNullable<Awaited<ReturnType<typeof getDb>>>, workspaceId: number, userId: number, subject: string, details: Record<string, unknown>) {
  await db.insert(auditEvents).values({
    ...
    evidenceHash: `${workspaceId}:${userId}:${subject}:${JSON.stringify(details)}`.slice(0, 128),
  });
}

type Checkpoint = { completedTaskIds: number[]; failedTaskIds: number[]; blockedTaskIds?: number[]; nextTaskIndex: number };

function parseCheckpoint(value: string): Checkpoint {
  try {
    const parsed = JSON.parse(value) as Partial<Checkpoint>;
    return {
      completedTaskIds: Array.isArray(parsed.completedTaskIds) ? parsed.completedTaskIds.filter(Number.isInteger) : [],
      failedTaskIds: Array.isArray(parsed.failedTaskIds) ? parsed.failedTaskIds.filter(Number.isInteger) : [],
      blockedTaskIds: Array.isArray(parsed.blockedTaskIds) ? parsed.blockedTaskIds.filter(Number.isInteger) : [],
      nextTaskIndex: Number.isInteger(parsed.nextTaskIndex) && Number(parsed.nextTaskIndex) >= 0 ? Number(parsed.nextTaskIndex) : 0,
    };
  } catch {
...
export function reconcileCompletedTaskIds(taskIds: readonly number[], checkpointIds: readonly number[]): Set<number> {
  const validIds = new Set(taskIds);
  return new Set(checkpointIds.filter(id => validIds.has(id)));
}

export function selectDependencyReadyTask<T extends { id: number; status: string; priority: number }>(tasks: T[], dependencies: Array<{ taskId: number; dependsOnTaskId: number }>, completedIds: Set<number>) {
  const dependencyMap = new Map<number, number[]>();
  for (const dependency of dependencies) dependencyMap.set(dependency.taskId, [...(dependencyMap.get(dependency.taskId) ?? []), dependency.dependsOnTaskId]);
  return tasks.filter(task => task.status === "queued").filter(task => (dependencyMap.get(task.id) ?? []).every(dependencyId => completedIds.has(dependencyId))).sort((a, b) => b.priority - a.priority || a.id - b.id)[0];
}

export function hasDependencyCycle(taskIds: readonly number[], dependencies: Array<{ taskId: number; dependsOnTaskId: number }>): boolean {
  ...
  const visit = (id: number): boolean => {
    ...
    visiting.add(id);
    for (const dependencyId of graph.get(id) ?? []) if (visit(dependencyId)) return true;
    visiting.delete(id);
    visited.add(id);
    return false;
  };
...
export async function executePlaybookRunJob(payload: Record<string, unknown>): Promise<PlaybookExecutionResult> {
  const runId = Number(payload.runId);
  if (!Number.isInteger(runId) || runId <= 0) throw new Error("Invalid playbook run job payload.");
  ...
  if (["completed", "cancelled"].includes(run.status)) return run.status === "completed" ? { status: "completed", runId } : { status: "failed", runId, reason: "Playbook run was cancelled." };
  if (!["queued", "running", "paused"].includes(run.status)) return { status: "failed", runId, reason: `Playbook run is not executable from status '${run.status}'.` };

  const taskIdsValue = JSON.parse(run.taskIds) as unknown;
  if (!Array.isArray(taskIdsValue) || taskIdsValue.some(id => !Number.isInteger(id))) throw new Error("Playbook task list is invalid.");
  ...
  if (dependencies.some(dependency => !ids.includes(dependency.dependsOnTaskId))) {
  ...
  if (!task) {
    ...
    await db.update(playbookRuns).set({ status: "paused", checkpoint: JSON.stringify({ ...checkpoint, blockedTaskIds: ids.filter(id => !completedIds.has(id)) }), lastError: reason, updatedAt: new Date() }).where(eq(playbookRuns.id, run.id));
  ...
  await db.update(researchTasks).set({ status: "blocked", outputs: JSON.stringify({ status: "blocked", reason, requiresAdapter: true }), updatedAt: new Date() }).where(and(eq(researchTasks.id, task.id), eq(researchTasks.workspaceId, run.workspaceId), eq(researchTasks.status, "queued")));
  await db.update(playbookRuns).set({ status: "paused", checkpoint: JSON.stringify({ ...checkpoint, blockedTaskIds: [...(checkpoint.blockedTaskIds ?? []), task.id] }), lastError: reason, updatedAt: new Date() }).where(eq(playbookRuns.id, run.id));
...
export function playbookJobPayload(runId: number) { return { type: "playbook_run", runId }; }
