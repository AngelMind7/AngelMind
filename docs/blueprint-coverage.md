...
| 41 | Notification | **Partial** | In-app notifications/preferences, cursor polling, mention delivery, safe webhook boundary, notification audit, delivery ledger, and persisted per-event email opt-in state are implemented; generic notification queue, provider delivery integration, retry/failure status, and signed unsubscribe links remain open. |
