...
export default function EvidenceVault() {
  ...
  const evidence = trpc.evidence.listPage.useQuery({ workspaceId: selectedWorkspaceId!, pageSize: 25, cursor }, { enabled: Boolean(selectedWorkspaceId) });
  const lineage = trpc.evidence.lineage.useQuery({ evidenceArtifactId: selectedId! }, { enabled: Boolean(selectedId) });
  const replay = trpc.evidence.provenanceReplay.useQuery({ evidenceArtifactId: selectedId! }, { enabled: Boolean(selectedId) });
  const utils = trpc.useUtils();
  const provenance = trpc.evidence.recordProvenance.useMutation({
    onSuccess: () => {
      setSourceReference("");
      void utils.evidence.listPage.invalidate();
      void utils.evidence.lineage.invalidate();
      void utils.evidence.provenanceReplay.invalidate();
    },
  });

  useEffect(() => {
    if (!selectedId && evidence.data?.items[0]?.id) setSelectedId(evidence.data.items[0].id);
    if (selectedId && evidence.data && !evidence.data.items.some(item => item.id === selectedId)) setSelectedId(evidence.data.items[0]?.id);
    ...
    <NeonFrame className="p-5 sm:p-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <label className="grid gap-2 text-xs uppercase tracking-[.12em] text-slate-400">Workspace
          <select aria-label="Evidence workspace" value={selectedWorkspaceId ?? ""} onChange={event => { setWorkspaceId(Number(event.target.value)); setCursor(undefined); setSelectedId(undefined); }} className="h-10 min-w-64 border border-cyan-300/20 bg-[#0a0d19] px-3 text-sm normal-case tracking-normal text-slate-200"><option value="">Select workspace</option>{workspaces.data?.map(workspace => <option key={workspace.id} value={workspace.id}>{workspace.name}</option>)}</select>
        </label>
