...
export default function MissionControl() {
  ...
  const handleRealtimeEvent = useCallback((event: RealtimeEvent) => {
    if (event.eventType?.startsWith("execution.")) setExecution(event);
  }, []);
  ...
  return <div className="mx-auto max-w-7xl space-y-6">
    ...
    {execution && <NeonFrame className="p-5 sm:p-6"><div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><div><Eyebrow>Governed execution</Eyebrow><h2 className="mt-2 font-display text-2xl font-bold text-white">Execution #{execution.aggregateId ?? "—"}</h2></div><Badge variant="outline" className="w-fit border-cyan-300/25 text-cyan-200">{execution.payload?.state ? String(execution.payload.state) : execution.eventType}</Badge></div><div className="mt-4 grid gap-3 text-sm sm:grid-cols-3"><div><span className="font-mono text-[10px] uppercase tracking-[.12em] text-slate-500">Capability</span><p className="mt-1 text-slate-200">{String(execution.payload?.capability ?? "—")}</p></div><div><span className="font-mono text-[10px] uppercase tracking-[.12em] text-slate-500">Tool</span><p className="mt-1 text-slate-200">{String(execution.payload?.toolKey ?? "—")}</p></div><div><span className="font-mono text-[10px] uppercase tracking-[.12em] text-slate-500">Revision</span><p className="mt-1 font-mono text-cyan-200">{String(execution.payload?.revision ?? "—")}</p></div></div></NeonFrame>}
