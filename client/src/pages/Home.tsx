  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
      <header className="relative overflow-hidden px-1 py-5 sm:py-8">
        ...
        <Eyebrow>Authorized security research · internal command interface</Eyebrow>
        <div className="mt-3 flex flex-col justify-between gap-5 lg:flex-row lg:items-end">
          <div><h1 className="font-display text-4xl font-black uppercase tracking-[-0.055em] text-white sm:text-6xl">Command <span className="neon-pink">Center</span></h1><p className="mt-3 max-w-2xl text-sm leading-6 text-slate-400">A governed control plane for scoped workspaces. Rehearsals remain fully offline; every future action is evaluated by deterministic safety policy before it can proceed.</p></div>
          <WorkspaceSetupDialog />
        </div>
      </header>

      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
        <Metric icon={BoxesIcon} label="Active workspaces" value={String(dashboard.data?.activeWorkspaceCount ?? 0)} tone="cyan" />
        <Metric icon={ShieldAlert} label="Pending approvals" value={String(dashboard.data?.pendingApprovalCount ?? 0)} tone="pink" />
        <Metric icon={Bot} label="Policy blocks" value={String(dashboard.data?.policyBlockCount ?? 0)} tone="orange" />
        <Metric icon={FileCheck2} label="Validated findings" value={String(dashboard.data?.validatedFindingCount ?? 0)} tone="cyan" />
        <Metric icon={CircleDollarSign} label="Recorded spend" value={cents(dashboard.data?.estimatedSpendCents ?? 0)} tone="pink" />
      </div>

      <div className="grid gap-6 xl:grid-cols-[1.32fr_.68fr]">
        <NeonFrame className="min-h-[410px] p-5 sm:p-6">
          <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start"><div><Eyebrow>Network-zero rehearsal</Eyebrow><h2 className="mt-2 font-display text-2xl font-bold text-white">Plan safely before anything else</h2></div><Badge className="border border-cyan-300/30 bg-cyan-300/10 font-mono text-[10px] uppercase tracking-[.16em] text-cyan-200">Dry-run only</Badge></div>
          <div className="mt-6 grid gap-4 sm:grid-cols-2"><ControlItem icon={ShieldCheck} title="Scope + conduct gate" detail="The declared allowlist and exclusions are evaluated deterministically." /><ControlItem icon={Braces} title="No target interaction" detail="Network calls and tool executions remain fixed at zero for every rehearsal." /><ControlItem icon={Clock3} title="Budget + session checks" detail="Estimates are blocked if the workspace budget or session ceiling is exhausted." /><ControlItem icon={Sparkles} title="Hypothetical task map" detail="Only policy, asset-graph, and coverage planning tasks are generated." /></div>
          <div className="mt-6 flex flex-col gap-3 border-t border-cyan-300/15 pt-5 sm:flex-row sm:items-center">
            <select aria-label="Select workspace for rehearsal" value={workspaceId ?? ""} onChange={event => setSelectedWorkspaceId(Number(event.target.value))} className="h-10 flex-1 rounded-sm border border-cyan-300/20 bg-[#0a0d19] px-3 text-sm text-slate-200 outline-none focus:border-cyan-300" disabled={!workspaces.data?.length}><option value="">{workspaces.data?.length ? "Select guarded workspace" : "Create a workspace to begin"}</option>{workspaces.data?.map(workspace => <option key={workspace.id} value={workspace.id}>{workspace.name} · {workspace.status}</option>)}</select>
            <Button className="neon-button min-w-44" disabled={!workspaceId || runRehearsal.isPending} onClick={() => workspaceId && runRehearsal.mutate({ workspaceId })}>{runRehearsal.isPending ? "Rehearsing…" : "Start rehearsal"}</Button>
          </div>
        ...
        <NeonFrame className="p-5 sm:p-6"><Eyebrow>Operational telemetry</Eyebrow><h2 className="mt-2 font-display text-2xl font-bold text-white">Coverage lens</h2><p className="mt-2 text-sm leading-6 text-slate-400">Coverage indicators remain empty until a governed run records eligible, reviewable evidence.</p><div className="mt-7 space-y-5"><Coverage label="Scope governance" value={selectedWorkspace ? 100 : 0} color="bg-cyan-300" /><Coverage label="Program context" value={selectedWorkspace ? 100 : 0} color="bg-fuchsia-400" /><Coverage label="Hypothesis planning" value={rehearsal ? 100 : 0} color="bg-cyan-300" /><Coverage label="Finding validation" value={0} color="bg-fuchsia-400" /></div><div className="mt-7 border-t border-cyan-300/15 pt-5 font-mono text-[10px] uppercase tracking-[.15em] text-slate-500">No model execution · no external target contact</div></NeonFrame>
      </div>

      <NeonFrame className="p-5 sm:p-6"><div className="flex items-center justify-between gap-4"><div><Eyebrow>Run ledger</Eyebrow><h2 className="mt-2 font-display text-2xl font-bold text-white">Latest governed activity</h2></div><Activity className="h-5 w-5 text-cyan-300" /></div><div className="mt-5 overflow-x-auto"><table className="w-full min-w-[650px] text-left text-sm"><thead className="font-mono text-[10px] uppercase tracking-[.14em] text-slate-500"><tr><th className="pb-3 font-normal">Mode</th><th className="pb-3 font-normal">Status</th><th className="pb-3 font-normal">Tier</th><th className="pb-3 font-normal">Tasks</th><th className="pb-3 font-normal">Estimate</th><th className="pb-3 font-normal">Created</th></tr></thead><tbody>{dashboard.data?.recentRuns.length ? dashboard.data.recentRuns.map(run => <tr className="border-t border-cyan-300/10" key={run.id}><td className="py-3 text-slate-300">{run.mode.replace("_", " ")}</td><td className="py-3"><StatusBadge value={run.status} /></td><td className="py-3 font-mono text-xs text-fuchsia-300">{run.governanceTier}</td><td className="py-3 text-slate-300">{run.plannedTaskCount}</td><td className="py-3 text-slate-300">{cents(run.estimatedCostCents)}</td><td className="py-3 text-slate-500">{new Date(run.createdAt).toLocaleString()}</td></tr>) : <tr><td colSpan={6} className="py-10 text-center text-slate-500">No governed activity has been recorded yet.</td></tr>}</tbody></table></div></NeonFrame>
    </div>
  );
}
