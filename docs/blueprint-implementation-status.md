...
Schema terbaru berada pada rangkaian migration sampai `drizzle/0029_evidence_quarantine_lifecycle.sql`, termasuk identity, tenant, research lineage foreign keys, AI run lineage, relational task dependencies, worker lease/heartbeat fields, outbox consumer receipts, evidence quarantine lifecycle, failure observations, evolution snapshots, intelligence feeds, playbooks, evidence provenance, reports, collaboration, outbox versioning, AI evaluations, dan AI run retention. Jalankan migration pada environment yang memiliki `DATABASE_URL` setelah meninjau SQL tersebut; sandbox tidak mengeksekusi migration ke database produksi.

## Validasi

Validasi branch main mencakup `pnpm check`, seluruh test suite Vitest dengan 44 test files dan 98 tests, `pnpm build`, `pnpm check:budget`, `pnpm lint:python`, `pnpm test:python`, provider-neutral check, migration journal check, dan `git diff --check`. V2 menambahkan tests untuk ordering/readiness, failure validation, snapshot evolution diff, playbook matching, intelligence feed normalization, route cleanup, dan localization inventory. Build dapat memberikan warning tooling yang tidak menggagalkan proses.

## Batas yang disengaja

Blueprint tetap lebih besar daripada satu batch implementasi. Fitur yang belum diklaim selesai antara lain payment billing provider, passkey/MFA native end-to-end, search ranking/semantic index, full collaboration/review UI, complete REST versioning, production secret-backed gateway health probes, malware scanner provider, disaster recovery drill terotomasi, dan seluruh critical-path E2E. Rehearsal tetap offline dan aplikasi tidak melakukan target-facing execution otonom.
