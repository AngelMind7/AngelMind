...
export function getRunEligibility(workspace: RunEligibilityWorkspace, now = Date.now()): { eligible: boolean; reason?: "workspace-not-active" | "cooldown" | "budget" | "session-limit" } {
  if (!workspace || !["active", "paused", "archived"].includes(workspace.status)) return { eligible: false, reason: "workspace-not-active" };
  ...
  if (workspace.lastRunAt && (!(workspace.lastRunAt instanceof Date) || !Number.isFinite(workspace.lastRunAt.getTime()))) return { eligible: false, reason: "cooldown" };
