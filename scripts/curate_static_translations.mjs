...
const source = JSON.parse(await readFile(path.join(root, "apps/frontend-angular/src/locales/static-translations.json"), "utf8"));
const locales = ["id", "ms", "ar", "zh-CN", "ja", "ko", "es", "pt", "fr", "de", "ru"];
const sourceFiles = [];
for await (const file of glob("apps/frontend-angular/src/**/*.{tsx,ts}", { cwd: root })) sourceFiles.push(file);
...
await writeFile(path.join(root, "apps/frontend-angular/src/locales/curated-translations.json"), JSON.stringify(curated, null, 2) + "\n");
