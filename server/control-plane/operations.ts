...
export function roleAllowsWorkspaceAccess(
  role: WorkspaceRole,
...
export function ownerMembershipRecord(
  workspaceId: number,
  ownerUserId: number
) {
  return {
...
async function ownedWorkspaceOrThrow(userId: number, workspaceId: number) {
  ...
  if (!workspace)
    throw new Error("Workspace tidak ditemukan atau tidak dapat dikelola.");
  return workspace;
}

async function workspaceAccessOrThrow(
  userId: number,
  workspaceId: number,
  intent: WorkspaceAccessIntent
) {
  if (!(await canAccessWorkspace(userId, workspaceId, intent)))
    throw new Error(
      "Workspace tidak ditemukan atau role tidak memiliki akses untuk operasi ini."
    );
...
export async function ensureOwnerMembership(
  ...
  await db
    ...
    .onDuplicateKeyUpdate({
      set: { role: "owner", addedByUserId: ownerUserId },
    });
}
...
export async function addMember(
  ownerUserId: number,
  input: { workspaceId: number; email: string; role: MemberRole }
) {
  const workspace = await ownedWorkspaceOrThrow(ownerUserId, input.workspaceId);
  const user = await getUserByEmail(input.email);
  if (!user)
    throw new Error(
      "User harus login sekali terlebih dahulu agar dapat ditambahkan melalui email."
    );
  const db = await getDb();
  if (!db) throw new Error("Database tidak tersedia.");
  await db
    ...
    .onDuplicateKeyUpdate({
      set: { role: input.role, addedByUserId: ownerUserId },
    });
  await addAudit(workspace.id, "membership", "workspace-member-upserted", {
...
export async function removeMember(
  ...
  if (membership.role === "owner")
    throw new Error("Owner membership cannot be removed from this interface.");
  await db
    .delete(workspaceMemberships)
    .where(eq(workspaceMemberships.id, membership.id));
  await addAudit(workspace.id, "membership", "workspace-member-removed", {
    memberUserId: membership.userId,
    removedByUserId: ownerUserId,
  });
  return { success: true };
}
  ...
  const memberships = await db
    ...
    .where(
      and(
        ...
        eq(workspaceMemberships.role, "reviewer")
      )
    );
  return memberships
    .filter(membership => membership.workspaceId > 0)
    .map(membership => membership.workspaceId);
}

export async function getApprovalWorkspaceIds(userId: number) {
  const db = await getDb();
  if (!db) return [];
  const memberships = await db
    .select({ workspaceId: workspaceMemberships.workspaceId })
    ...
    .where(
      and(
        eq(workspaceMemberships.userId, userId),
        inArray(workspaceMemberships.role, ["reviewer", "approval_authority"])
      )
    );
  return Array.from(
    new Set(
...
export async function getReadableWorkspaceIds(userId: number) {
  ...
  const memberIds = memberships
    .filter(membership => roleAllowsWorkspaceAccess(membership.role, "read"))
    .map(membership => membership.workspaceId);
  return ownedIds.concat(
    memberIds.filter(workspaceId => !ownedIds.includes(workspaceId))
  );
}
...
export async function hasApprovalAuthorityMembership(
  userId: number,
  workspaceId: number
) {
  const db = await getDb();
  ...
  const [membership] = await db
    ...
    .where(
      and(
        eq(workspaceMemberships.workspaceId, workspaceId),
        eq(workspaceMemberships.userId, userId),
        eq(workspaceMemberships.role, "approval_authority")
      )
    )
    .limit(1);
  return Boolean(membership);
...
export async function getWebhookConfiguration(
  ...
  const [configuration] = await db
    ...
    .where(eq(webhookConfigurations.workspaceId, workspaceId))
    .limit(1);
...
export async function saveWebhookDraft(
  ...
  if (
    input.signingSecretReference &&
    !input.signingSecretReference.startsWith(
      `secret://workspace-${workspace.id}/webhook-`
    )
  )
    throw new Error(
      "Webhook signing secret must use this workspace's secret namespace."
    );
  const db = await getDb();
  if (!db) throw new Error("Database tidak tersedia.");
  await db
    ...
    .values({
      workspaceId: workspace.id,
      endpoint,
      signingSecretReference: input.signingSecretReference || null,
      eventTypes: JSON.stringify(eventTypes),
      endpointConfirmed: input.endpointConfirmed ? 1 : 0,
      enabled: 0,
  ...
  await addAudit(workspace.id, "webhook", "webhook-draft-saved", {
    endpointHost: new URL(endpoint).hostname,
    eventTypes,
    endpointConfirmed: input.endpointConfirmed,
    enabled: false,
  });
...
export async function createAuditArchive(
  ...
  const db = await getDb();
  if (!db) throw new Error("Database tidak tersedia.");
  if (!ENV.archiveSigningSecret)
    throw new Error("Archive signing secret is unavailable.");
  const [events, evidence, runRows, approvalRows, notificationRows] =
    await Promise.all([
      db
        .select()
        .from(auditEvents)
  ...
  const manifestJson = JSON.stringify({
    schema: "angelmind.audit-archive.v1",
    workspaceId: workspace.id,
  ...
  const retentionUntil = new Date(
    Date.now() + workspace.retentionDays * 86_400_000
  );
  const stored = await storagePut(
    `workspace-${workspace.id}/audit-archives/${Date.now()}-manifest.json`,
    manifestJson,
    "application/json"
  );
  await db
...
export async function listAuditArchives(userId: number, workspaceId: number) {
  ...
  return db
    .select()
    .from(auditArchives)
    .where(eq(auditArchives.workspaceId, workspaceId))
    .orderBy(desc(auditArchives.createdAt));
}

export async function restoreAuditArchivePlan(
  ownerUserId: number,
  archiveId: number,
  ...
  if (!response.ok)
    throw new Error("Archive could not be retrieved from managed storage.");
  const manifestJson = await response.text();
  const valid = verifyArchiveIntegrity(
    manifestJson,
    archive.manifestHash,
    archive.signature,
    ENV.archiveSigningSecret
  );
  if (!valid)
    throw new Error(
      "Archive integrity verification failed; restore plan refused."
    );
  const manifest = assertArchiveManifest(manifestJson, archive.workspaceId) as {
    schema?: unknown;
    workspaceId?: unknown;
    auditEvents?: unknown[];
    evidence?: unknown[];
  ...
  return {
    ...
    recordCounts: {
      ...
      evidence: manifest.evidence?.length ?? 0,
      runs: manifest.runs?.length ?? 0,
      approvals: manifest.approvals?.length ?? 0,
      notifications: manifest.notifications?.length ?? 0,
    },
...
export async function verifyAuditArchive(
  ...
  if (valid)
    await db
      ...
      .where(eq(auditArchives.id, archive.id));
  await addAudit(archive.workspaceId, "audit-archive", "archive-verified", {
...
export async function runAuditArchiveDrill(
  ownerUserId: number,
  archiveId: number,
  destinationWorkspaceId?: number,
  idempotencyKey = `restore-drill:${archiveId}:${Date.now()}`
) {
  const db = await getDb();
  if (!db) throw new Error("Database tidak tersedia.");
  ...
  const [existing] = await db
    ...
    .where(
      and(
        ...
        eq(restoreDrillRuns.idempotencyKey, key)
      )
    )
    .limit(1);
  if (existing) {
    if (existing.status === "running")
      throw new Error(
        "A restore drill with this idempotency key is already running."
      );
    if (existing.status === "failed")
      throw new Error(
        existing.errorMessage ||
          "The previous restore drill failed; use a new key."
      );
    return {
      archiveId,
      valid: existing.valid === 1,
      mode: "plan-only" as const,
      requiresHumanConfirmation: true as const,
      recordsChecked: JSON.parse(existing.recordsChecked),
  ...
  try {
    ...
    if (concurrent.status === "failed")
      throw new Error(
        concurrent.errorMessage ||
          "The previous restore drill failed; use a new key."
      );
    return {
      archiveId,
      valid: concurrent.valid === 1,
      mode: "plan-only" as const,
      requiresHumanConfirmation: true as const,
      recordsChecked: JSON.parse(concurrent.recordsChecked),
      mutationPerformed: false as const,
      rollbackRequired: false as const,
      replayed: true as const,
    };
  ...
  try {
    ...
    if (!verification.valid)
      throw new Error("DR drill refused because archive integrity is invalid.");
    const plan = await restoreAuditArchivePlan(
      ownerUserId,
      archiveId,
