...
export const appRouter = router({
  ...
  agent: router({
    ...
    startDurableRun: protectedProcedure.input(z.object({ workspaceId: z.number().int().positive(), sessionId: z.number().int().positive().optional(), taskId: z.number().int().positive().optional(), modelKey: z.string().trim().min(2).max(160), gateway: z.string().trim().min(2).max(120), purpose: z.string().trim().min(2).max(120), inputReference: z.string().trim().min(1).max(512), messages: z.array(z.object({ role: z.enum(["system", "user", "assistant"]), content: z.string().max(100_000) })).min(1).max(50), estimatedCostCents: z.number().int().min(0).max(100_000_000).optional(), retentionDays: z.number().int().min(1).max(3_650).optional(), idempotencyKey: z.string().trim().min(8).max(180) })).mutation(({ ctx, input }) => aiPlatform.startDurableAiRun(ctx.user.id, input)),
