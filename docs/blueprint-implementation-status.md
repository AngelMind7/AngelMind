...
| Platform dan integrations | Sebagian besar tersedia | tRPC API, Firebase, Supabase Storage, Railway deployment/Cron, GitHub CI, webhook policy, audit archive, provider-neutral guard, API key authentication fallback, privacy request lifecycle, cursor-backed global search, dan mutation-triggered search consistency. Versioned REST gateway, semantic retrieval, dan collaboration lanjutan masih perlu iterasi. |
| Operations dan governance | Sebagian besar tersedia | Health, metrics, Docker, Railway health check, scheduled maintenance, backup/archive signing, incident workflow, approval workflow, notification, secret references, optimistic finding concurrency, dan CI guard tersedia. Billing eksternal belum diaktifkan karena provider payment belum ditentukan. |
| Frontend quality | Sebagian besar tersedia | React lazy routes, responsive UI, i18n inventory, PWA, loading/error/empty states, accessibility primitives, Security Center, Organization Hub, Research Workspace, AI Center, Profile page, dan finding remediation controls. E2E critical path dan bundle optimization masih perlu iterasi. |
...
Schema terbaru berada pada rangkaian migration sampai `src/c2/database/0063_observation_provenance.sql`, termasuk identity, tenant, research lineage foreign keys, AI run lineage, relational task dependencies, worker lease/heartbeat fields, outbox consumer receipts, evidence quarantine lifecycle, failure observations, evolution snapshots, intelligence feeds, playbooks, evidence provenance, observation raw-output/normalized-evidence hashes, reports, collaboration, outbox versioning, AI evaluations, AI run retention, finding remediation metadata, dan optimistic finding revisions. Jalankan migration pada environment yang memiliki `DATABASE_URL` setelah meninjau SQL tersebut; sandbox tidak mengeksekusi migration ke database produksi.

## Validasi

Validasi branch main mencakup `pnpm check`, seluruh test suite Vitest, `pnpm build`, `pnpm check:budget`, `pnpm lint:python`, `pnpm test:python`, provider-neutral check, migration journal/safety/rollback checks, dan `git diff --check`. Finding workflow tests kini mencakup notification/remediation/retest/resolution/reopen checkpoints serta immutable submission boundary. Build dapat memberikan warning tooling yang tidak menggagalkan proses.

## Batas yang disengaja

Blueprint tetap lebih besar daripada satu batch implementasi. Fitur yang belum diklaim selesai antara lain payment billing provider, passkey/MFA native end-to-end, semantic search retrieval, full collaboration/review UI, complete REST versioning, production secret-backed gateway health probes, malware scanner provider, disaster recovery drill terotomasi, dan seluruh critical-path E2E. Rehearsal tetap offline dan aplikasi tidak melakukan target-facing execution otonom.
