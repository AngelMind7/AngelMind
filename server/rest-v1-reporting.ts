...
export function registerRestV1ReportingRoutes(app: Express) {
  ...
  app.post("/api/v1/reports/:id/status", async (req,res) => { try { await auth(req); ok(res,reporting.updateReportStatus(String(req.params.id),req.body?.status)); } catch(e){fail(res,e);} });
  app.get("/api/v1/reports/:id/export", async (req,res) => { try { await auth(req); ok(res,reporting.exportReport(String(req.params.id))); } catch(e){fail(res,e);} });
}
