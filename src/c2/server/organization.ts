...
import { organizationAuditEvents, organizationInvitations, programScopeVersions, programs, organizationMembers, organizations, workspaces } from "../database/schema";
...
export async function acceptOrganizationInvitation(userId: number, token: string) {
  ...
  const user = await db.select().from((await import("../database/schema")).users).where(eq((await import("../database/schema")).users.id, userId)).limit(1);
