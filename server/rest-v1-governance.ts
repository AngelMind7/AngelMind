...
import { createIncident, createPolicy, decideApproval, escalateApproval, listApprovals, listIncidents, listPolicies, updateIncident, verifyAuditIntegrity, appendAuditEvent, buildComplianceMap, buildRiskAssessment, buildVendorAssessment } from "./governance-audit";
...
export function registerGovernanceRestV1Routes(app: Express) {
  app.get("/api/v1/governance/policies", async (req, res) => { try { const uid = await auth(req); res.json({ policies: await listPolicies(uid, Number(req.query.workspaceId)) }); } catch (e) { res.status(403).json({ error: e instanceof Error ? e.message : "Forbidden" }); } });
  app.post("/api/v1/governance/policies", async (req, res) => { try { const uid = await auth(req); res.status(201).json(await createPolicy(uid, req.body)); } catch (e) { res.status(400).json({ error: e instanceof Error ? e.message : "Bad request" }); } });
  app.get("/api/v1/governance/approvals", async (req, res) => { try { const uid = await auth(req); res.json({ approvals: await listApprovals(uid, Number(req.query.workspaceId)) }); } catch (e) { res.status(403).json({ error: e instanceof Error ? e.message : "Forbidden" }); } });
  ...
  app.post("/api/v1/governance/approvals/:id/escalate", async (req, res) => { try { const uid = await auth(req); res.json(await escalateApproval(uid, { id: Number(req.params.id), note: String(req.body?.note ?? "") })); } catch (e) { res.status(400).json({ error: e instanceof Error ? e.message : "Bad request" }); } });
  ...
  app.post("/api/v1/incidents", async (req, res) => { try { const uid = await auth(req); res.status(201).json(await createIncident(uid, { ...req.body, escalationDueAt: new Date(req.body.escalationDueAt) })); } catch (e) { res.status(400).json({ error: e instanceof Error ? e.message : "Bad request" }); } });
  app.patch("/api/v1/incidents/:id", async (req, res) => { try { const uid = await auth(req); res.json(await updateIncident(uid, { id: Number(req.params.id), status: req.body.status, resolutionNote: req.body.resolutionNote })); } catch (e) { res.status(400).json({ error: e instanceof Error ? e.message : "Bad request" }); } });
  app.get("/api/v1/governance/compliance/:framework", async (req, res) => { if (!governanceFrameworks.includes(req.params.framework as any)) return res.status(400).json({ error: "Unsupported framework" }); res.json(buildComplianceMap(req.params.framework as any)); });
