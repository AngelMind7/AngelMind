...
| 97 | Supply-chain security | **Partial** | Lockfile, dependency review/security workflow, pinned CycloneDX SBOM generation/upload, pinned HIGH/CRITICAL Trivy image scan evidence, keyless Cosign signing/verification on main image pushes, and container hardening are implemented; full DAST and production registry retention policy remain open. |
