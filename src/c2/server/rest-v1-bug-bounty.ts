...
export function registerBugBountyRestV1Routes(app: Express) {
  ...
  app.post("/api/v1/bugbounty/programs", async (req, res) => { try { const id = await uid(req); const wid = workspaceId(req); if (wid > 0 && !(await canAccessWorkspace(id, wid, "manage"))) return res.status(403).json({ error: "Forbidden" }); res.status(201).json(createBountyProgram(req.body)); } catch (e) { fail(res, e); } });
