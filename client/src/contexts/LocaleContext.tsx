...
type LocaleContextValue = { locale: LocaleCode; setLocale: (locale: LocaleCode) => void; t: (key: TranslationKey) => string; copy: (key: HomeCopyKey) => string; formatDate: (value: Date | string | number) => string; formatNumber: (value: number) => string; isRTL: boolean };
...
export function translate(locale: LocaleCode, key: TranslationKey): string { return packs[locale][key] || english[key]; }
...
export function localizeEmbeddedTimestamp(locale: LocaleCode, value: string): string {
  const pattern = /\b\d{1,2}\/\d{1,2}\/\d{4},?\s+\d{1,2}:\d{2}(?::\d{2})?\s+[AP]M\b/g;
  return value.replace(pattern, source => {
    const parsed = new Date(source);
    return Number.isNaN(parsed.getTime()) ? source : new Intl.DateTimeFormat(locale, { dateStyle: "medium", timeStyle: "short" }).format(parsed);
  });
}
  ...
  useEffect(() => {
    const localize = (root: ParentNode) => {
      ...
      nodes.forEach(node => {
        ...
        const translated = localizeEmbeddedTimestamp(locale, translateStatic(locale, normalized));
...
export function LocaleProvider({ children }: { children: React.ReactNode }) {
  ...
  const value = useMemo(() => ({ locale, setLocale, isRTL: meta.dir === "rtl", t: (key: TranslationKey) => translate(locale, key), copy: (key: HomeCopyKey) => homeCopy[key]?.[locale] ?? homeCopy[key].en, formatDate: (input: Date | string | number) => new Intl.DateTimeFormat(locale, { dateStyle: "medium", timeStyle: "short" }).format(new Date(input)), formatNumber: (value: number) => new Intl.NumberFormat(locale).format(value) }), [locale, meta.dir]);
