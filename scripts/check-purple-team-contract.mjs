...
const failures = [];
for (const file of ["src/c2/server/purple-team.ts","src/c2/server/purple-team.test.ts","src/c2/server/rest-v1-purple-team.ts","docs/domain/08-purple-team.md"]) if (!existsSync(resolve(root, file))) failures.push(`missing ${file}`);
const service = read("src/c2/server/purple-team.ts");
...
const rest = read("src/c2/server/rest-v1-purple-team.ts");
...
const index = read("src/c2/server/_core/index.ts"); if (!index.includes("registerPurpleTeamRestV1Routes")) failures.push("purple-team REST routes are not registered");
const test = read("src/c2/server/purple-team.test.ts"); if (!test.includes("requires approval before execution")) failures.push("missing approval test");
