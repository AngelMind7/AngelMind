...
Retest outcomes `verified_fixed` dan `still_present`, Integrations settings UI, automatic reputation hooks untuk validation/review/evidence/retest, dan authenticated accessibility route inventory untuk knowledge/integrations/admin kini sudah dipublish. Sisa provider handshake, live verification, dan richer public profile tetap environment/provider-dependent.
...
| P1 | End-to-end trace graph correlation. | **Selesai untuk record inti**: `traceId` dipersist pada session, asset, observation, hypothesis, task, finding, evidence artifact, playbook task, dan research-task outbox event; propagasi ke provider eksternal tetap bergantung pada adapter/provider. |
| P1 | Worker dan outbox operational semantics. | **Selesai untuk core dispatcher dan research task slice**: job lease/heartbeat, stale recovery, retry/dead-letter, outbox lease, backoff, consumer receipt setelah sukses, evidence-scan handler, durable research-task enqueue/execution, passive adapter gate, observation persistence, dan task lifecycle events tersedia. Integration test database live, multi-process verification, broader adapter coverage, dan deployment replay drill masih memerlukan environment. |
...
| P1 | Permission-aware ranked search. | **Selesai untuk deterministic search**: workspace permission boundary, cross-domain index, token ranking, freshness scoring, cursor pagination, dan asset inventory indexing tersedia. Semantic/vector retrieval tetap merupakan enhancement opsional. |
