...
RUN DEBIAN_FRONTEND=noninteractive apt-get update \
    && apt-get install --no-install-recommends -y \
       binutils ca-certificates curl dnsutils file jq python3 python3-pip ripgrep unzip \
    ...
    && tar -xzf /tmp/chainsaw.tar.gz -C /tmp && install -m 0755 /tmp/chainsaw/chainsaw /usr/local/bin/chainsaw \
    ...
    && tar -xzf /tmp/gosec.tar.gz -C /tmp && install -m 0755 /tmp/gosec /usr/local/bin/gosec \
    ...
    && unzip -q /tmp/dnsx.zip -d /tmp/dnsx && install -m 0755 /tmp/dnsx/dnsx /usr/local/bin/dnsx \
    ...
    && unzip -q /tmp/subfinder.zip -d /tmp/subfinder && install -m 0755 /tmp/subfinder/subfinder /usr/local/bin/subfinder \
    ...
    && tar -xzf /tmp/trufflehog.tar.gz -C /tmp trufflehog && install -m 0755 /tmp/trufflehog /usr/local/bin/trufflehog \
    ...
    && corepack enable && corepack prepare pnpm@${PNPM_VERSION} --activate && pnpm --version \
