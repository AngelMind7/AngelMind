...
async function requireWorkspace(req: Request, mode: "read" | "write") { const id = await uid(req); const workspaceId = wid(req); if (!Number.isInteger(workspaceId) || workspaceId < 1) throw new Error("workspaceId must be positive"); if (!(await canAccessWorkspace(id, workspaceId, mode))) throw new Error("Forbidden"); return workspaceId; }
...
export function registerThreatIntelligenceRestV1Routes(app: Express) {
  ...
  app.post("/api/v1/workspaces/:workspaceId/threat-intel", async (req, res) => { try { const workspaceId = await requireWorkspace(req, "write"); res.status(201).json(ingestIntelligence({ ...req.body, workspaceId })); } catch (e) { fail(res, e); } });
