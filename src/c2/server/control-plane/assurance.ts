...
import { evidenceArtifacts, incidentEvidenceLinks, incidentReviews, incidents, notificationPreferences, notifications, policyVersions, webhookActivationRequests, webhookConfigurations, workspaces } from "../../database/schema";
...
async function addAudit(workspaceId: number, category: string, subject: string, details: Record<string, unknown>) {
  ...
  await db.insert((await import("../../database/schema")).auditEvents).values({ workspaceId, category, subject, traceId, details: ENV.auditStateEncryptionKey ? encryptAuditState(details, ENV.auditStateEncryptionKey) : JSON.stringify(details), evidenceHash: sha256(JSON.stringify({ workspaceId, category, subject, details, traceId })) });
}
