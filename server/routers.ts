...
export const appRouter = router({
  ...
  agent: router({
    ...
    selectModel: protectedProcedure.input(z.object({ capabilities: z.array(z.string().trim().min(1).max(80)).max(30).optional(), minimumContextWindow: z.number().int().min(0).max(10_000_000).optional(), maxCostCentsPerMillionTokens: z.number().int().min(0).max(10_000_000).optional(), allowDegraded: z.boolean().optional() })).query(({ input }) => aiPlatform.selectRegisteredModel(input)),
    enqueuePlan: protectedProcedure.input(z.object({ workspaceId: z.number().int().positive(), objective: z.string().trim().min(10).max(10_000), roles: z.array(z.enum(["scope", "evidence", "risk", "report"])).min(1).max(4), evidenceReferences: z.array(z.string().trim().min(1).max(512)).max(100).optional(), idempotencyKey: z.string().trim().min(8).max(180) })).mutation(({ ctx, input }) => aiPlatform.enqueueOrchestrationPlan(ctx.user.id, input)),
    startDurableRun: protectedProcedure.input(z.object({ workspaceId: z.number().int().positive(), sessionId: z.number().int().positive().optional(), taskId: z.number().int().positive().optional(), modelKey: z.string().trim().min(2).max(160).optional(), capabilities: z.array(z.string().trim().min(1).max(80)).max(30).optional(), minimumContextWindow: z.number().int().min(0).max(10_000_000).optional(), maxCostCentsPerMillionTokens: z.number().int().min(0).max(10_000_000).optional(), allowDegraded: z.boolean().optional(), purpose: z.string().trim().min(2).max(120), inputReference: z.string().trim().min(1).max(512), messages: z.array(z.object({ role: z.enum(["system", "user", "assistant"]), content: z.string().max(100_000) })).min(1).max(50), estimatedCostCents: z.number().int().min(0).max(100_000_000).optional(), retentionDays: z.number().int().min(1).max(3_650).optional(), idempotencyKey: z.string().trim().min(8).max(180) })).mutation(({ ctx, input }) => aiPlatform.startDurableAiRun(ctx.user.id, input)),
    getRunOutput: protectedProcedure.input(z.object({ runId: z.number().int().positive() })).query(({ ctx, input }) => aiPlatform.getAiRunOutput(ctx.user.id, input.runId)),
    rebuildSearchIndex: protectedProcedure.input(z.object({ workspaceId: z.number().int().positive() })).mutation(({ ctx, input }) => globalSearch.rebuildWorkspaceSearchIndex(ctx.user.id, input.workspaceId)),
    ...
    analyzeEvidence: protectedProcedure.input(z.object({ workspaceId: z.number().int().positive(), scopeSummary: z.string().min(20).max(10_000), evidence: z.string().min(20).max(40_000), findingTitle: z.string().max(240).optional() })).mutation(({ ctx, input }) => agent.analyzeEvidenceForWorkspace(ctx.user.id, input)),
  ...
  research: router({
    ...
    failureObservations: protectedProcedure.input(z.object({ workspaceId: z.number().int().positive(), sessionId: z.number().int().positive().optional() })).query(({ ctx, input }) => researchIntelligence.listFailureObservations(ctx.user.id, input.workspaceId, input.sessionId)),
    createFailureObservation: protectedProcedure.input(z.object({ workspaceId: z.number().int().positive(), sessionId: z.number().int().positive(), kind: z.enum(["timeout", "dependency_failure", "partial_response", "error_state", "recovery_behavior", "retry_behavior", "concurrency", "race_condition", "transaction_failure", "degraded_mode", "cascading_failure"]), normalState: z.string().min(1).max(240), condition: z.string().min(1).max(20_000), observedBehavior: z.string().min(1).max(20_000), impact: z.enum(["none", "low", "medium", "high", "critical"]), evidenceRefs: z.array(z.string().min(1).max(512)).max(100) })).mutation(({ ctx, input }) => researchIntelligence.createFailureObservation(ctx.user.id, input)),
    evolutionSnapshots: protectedProcedure.input(z.object({ workspaceId: z.number().int().positive(), assetRef: z.string().max(512).optional() })).query(({ ctx, input }) => researchIntelligence.listEvolutionSnapshots(ctx.user.id, input.workspaceId, input.assetRef)),
    compareEvolution: protectedProcedure.input(z.object({ workspaceId: z.number().int().positive(), assetRef: z.string().min(1).max(512) })).query(({ ctx, input }) => researchIntelligence.compareLatestEvolution(ctx.user.id, input.workspaceId, input.assetRef)),
    createEvolutionSnapshot: protectedProcedure.input(z.object({ workspaceId: z.number().int().positive(), sessionId: z.number().int().positive().optional(), assetRef: z.string().min(1).max(512), version: z.string().min(1).max(120), capturedAt: z.coerce.date(), source: z.string().min(1).max(120), attributes: z.record(z.string(), z.union([z.string(), z.number(), z.boolean(), z.null()])) })).mutation(({ ctx, input }) => researchIntelligence.createEvolutionSnapshot(ctx.user.id, input)),
    intelligenceFeed: protectedProcedure.input(z.object({ workspaceId: z.number().int().positive(), assetRef: z.string().max(512).optional() })).query(({ ctx, input }) => researchIntelligence.listIntelligenceFeed(ctx.user.id, input.workspaceId, input.assetRef)),
    createIntelligenceFeed: protectedProcedure.input(z.object({ workspaceId: z.number().int().positive(), source: z.string().min(1).max(120), observedAt: z.coerce.date().transform(value => value.toISOString()), assetRef: z.string().min(1).max(512), confidence: z.number().min(0).max(100), reference: z.string().max(512).optional(), data: z.record(z.string(), z.unknown()) })).mutation(({ ctx, input }) => researchIntelligence.createIntelligenceFeedItem(ctx.user.id, input)),
    playbooks: protectedProcedure.input(z.object({ workspaceId: z.number().int().positive() })).query(({ ctx, input }) => researchIntelligence.listPlaybooks(ctx.user.id, input.workspaceId)),
    createPlaybook: protectedProcedure.input(z.object({ workspaceId: z.number().int().positive(), slug: z.string().min(1).max(160), version: z.string().min(1).max(40), status: z.enum(["draft", "active", "deprecated"]).optional(), domains: z.array(z.string().min(1).max(80)).max(50), assetTypes: z.array(z.string().min(1).max(80)).max(50), technologies: z.array(z.string().min(1).max(120)).max(100).optional(), taskTemplates: z.array(z.unknown()).max(200) })).mutation(({ ctx, input }) => researchIntelligence.createPlaybook(ctx.user.id, input)),
  }),
  rehearsal: router({
